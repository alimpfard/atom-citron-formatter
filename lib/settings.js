...
export default {
  ...
  getCitronExecutableSetting() {
    ...
    // returns true if project includes a .formatter.ctr file
    isFormatterFilePresent() {
      const path = main.getActiveTextEditorRootPath();
      return path && fs.existsSync(join(path, ".formatter.ctr"));
    },
    ...
    shouldIgnoreGrammar() {
      return atom.config.get("atom-citron-formatter.anygrammar");
    }
};
